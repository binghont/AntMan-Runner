...
//backgroundSprite->setScaleX(visibleSize.width);
    ...
    retryItem->setPosition( Point( visibleSize.width *3/4 + origin.x*1/4 , visibleSize.height / 2 ));
    ...
    mainMenuItem->setPosition( Point( visibleSize.width *3/4 + origin.x*2/3 , visibleSize.height / 4 ));
...
void GameOverScene::showScore( cocos2d::Layer *layer, int score ) {
    ...
    if ( score == 0 ) {
        ...
        label->setPosition( Point(score_positionX + origin.x, visibleSize.height*3/4 + origin.y));
        this->addChild(label);
    }
